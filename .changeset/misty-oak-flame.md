...
- `#!/usr/bin/env node` shebang in `src/cli.ts` is preserved naturally by tsdown/Rolldown into `dist/cli.mjs`; removed a `banner` injection that caused a doubled shebang in the published package
