...
- Add `deno.json` for `@mynameistito/create-cf-token` with exports mirroring `package.json` subpaths, `#src/` and runtime `npm:` import mappings, and `publish.exclude` aligned with the npm `files` field (source, automation assets, README, LICENSE)
- Add `deno.lock` for reproducible Deno dependency resolution in CI and local dry-runs
